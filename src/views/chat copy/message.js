...
const Message = {
    ...
    singleMsg: async (msg) => {
        ...
        if (!(await validFriend(store.getters.friend.list, femail))) return
        await store.dispatch('friend/pushMsg', { msg, email: femail })
    ...
    roomMsg: async (msg) => {
        ...
        // 解决方案2：等待好友和群列表加载完毕在发起websocket请求
        if (!(await validCommunity(store.getters.community.list, msg.dstObj))) return
        ...
        await store.dispatch('community/pushMsg', { msg, id: msg.dstObj })
    },
    // 上线消息
    onlineMsg: (msg) => {
